...
if ( ! class_exists('Fed_Cp_Menu')) {
	...
	class Fed_Cp_Menu{
		...
		public function __construct(){
			add_filter('fed_admin_dashboard_settings_menu_header', array($this, 'fed_cp_admin_dashboard_settings_menu_header'));
			add_filter('fed_get_custom_post_settings_by_type', array($this, 'fed_cp_get_custom_post_settings_by_type'), 10, 2);
			...
			add_action('fed_frontend_dashboard_menu_container', array($this, 'fed_cp_frontend_dashboard_menu_container'), 10, 2);

			add_action('wp_ajax_fed_dashboard_add_edit_post', array($this, 'fed_dashboard_add_edit_post'));
			add_action('wp_ajax_fed_dashboard_delete_post_by_id', array($this, 'fed_dashboard_delete_post_by_id_fn'));
		}
		...
		public function fed_dashboard_add_edit_post(){
			...
			if( count( array_intersect($user_role, array_keys($fed_admin_options['permissions']['post_permission'])) ) > 0 ){
				...
				if( empty($post['post_title']) ){
					$error = new WP_Error('fed_dashboard_add_post_title_missing', __('Please fill post title', 'frontend-dashboard-custom-post'));
					wp_send_json_error(array(
						'message' => $error->get_error_messages()
					));
				}
				...
				foreach ($extras as $index => $extra) {
					if( isset($extra['input_type']) && 'wp_editor' === $extra['input_type'] ) {
						...
						$default['meta_input'][$extra['input_meta']] = isset($post[$extra['input_meta']]) ? wp_kses($post[$extra['input_meta']], array()) : '';
					} else {
			...
			if ($admin_custom_post_options) {
				...
				foreach ($admin_custom_post_options as $key => $options) {
					if (in_array($key, array_keys($get_default_post_items))) {
						...
						if( isset($options['permissions']['post_permission']) 
							&& count(array_intersect($user->roles, array_keys($options['permissions']['post_permission']))) > 0 
						){
							$default[$key] = array(
								'id'                => $key,
								'menu_slug'         => $key,
				...
				if ($post_menus) {
					...
					<div class="panel panel-primary fed_dashboard_item active">
						...
						<div class="panel-body fed_dashboard_panel_body">
							...
							do_action( 'fed_dashboard_panel_inside_top_' . fed_get_data( 'menu_request.menu_slug', $menu_items ) );
							/**
							 * Add New post
							 */
							...
							if ( ! isset($request['post_status']) && ! isset($request['post_id'])) {
								$this->fed_display_dashboard_view_post_list($menu, $menu_items['menu_request']['menu_id']);
							}
			...
			if (count($tabs)) {
				...
				<div class="row">
					<div class="col-md-12 padd_top_20">
						<h5><?php esc_attr_e('Sorry! you don\'t have any public custom post type','frontend-dashboard-custom-post'); ?></h5>
					</div>
				</div>
				<?php
			}
				...
				<div class="fed_admin_panel_container">
					...
					<div class="fed_admin_panel_content_wrapper">
						...
						<div class="custom_post_dashboard">
							<div class="row">
								<div class="col-md-12">
									<h4><?php esc_attr_e( 'Dashboard Settings', 'frontend-dashboard-custom-post' ); ?></h4>
								</div>
							</div>
							<div class="row">
								<div class="col-md-4">
						...
						<div class="custom_post_menu">
							...
							<div class="row">
								<div class="col-md-4">
									<label><?php esc_attr_e('Post Menu Name','frontend-dashboard-custom-post'); ?></label>
									<?php
									// phpcs:ignore
								...
								<div class="col-md-4">
									<label><?php esc_attr_e( 'Post Menu Position', 'frontend-dashboard-custom-post' ) ?></label>
									<?php
									// phpcs:ignore
						...
						<div class="custom_post_permissions">
							<div class="row">
								<div class="col-md-12">
									<h4><?php esc_attr_e('Allow User Roles to Add/Edit/Delete Posts','frontend-dashboard-custom-post'); ?></h4>
								</div>
							</div>
							<div class="row">
								<?php
										...
										<div class="panel-body">
											<?php if (isset($tab['note'])) { ?>
												<div class="row p-b-20">
													<div class="col-md-12">
														<strong><?php echo esc_attr( isset( $tab['note'] ) ? $tab['note'] : '' ); ?></strong>
													</div>
												</div>
											<?php } ?>
		...
		public function fed_cp_admin_settings(){
			...
			if (fed_check_post_type($post_type)) {
				...
				$filter = apply_filters('fed_cp_admin_settings_save', $fed_admin_settings_custom_post, $post_type, $request);
		...
		public function fed_dashboard_delete_post_by_id_fn(){
			...
			if( !wp_verify_nonce($post_payload['fed_dashboard_delete_post_by_id'], 'fed_dashboard_delete_post_by_id') ){
				wp_send_json_error(array(
					'message' => array(
			...
			if( ( get_current_user_id() == $post->post_author || fed_is_admin() ) 
				&& fed_cp_is_user_can_delete_post($post->post_type) 
			){
		...
		private function fed_cp_frontend_dashboard_add_new_post($request, $menu){
			...
			if (
				...
				if ( 'pending' === $post_status ) {
					...
					$default_post_status = array( 'draft' => __('Draft') );
				}
			}
			...
			<form method="post"
				...
				<div class="row fed_dashboard_item_field">
					<div class="col-md-12">
						<div class="fed_header_font_color"><?php esc_attr_e( 'Title', 'frontend-dashboard-custom-post' ); ?></div>
						<?php
						// phpcs:ignore
				...
				if( !isset($post_settings['dashboard']['post_content']) && post_type_supports($post_type, 'editor') ){
					?><div class="row fed_dashboard_item_field">
						<div class="col-md-12">
							<div class="fed_header_font_color"><?php esc_attr_e( 'Content', 'frontend-dashboard-custom-post' ); ?></div>
		...
		private function fed_show_category_tag_post_format($post, $post_settings){
				...
				if ('post_format' === $index) {
					if ( ! isset($post_settings['taxonomies']['post_format'][$user_role])) {
						...
						if (is_array($post_format)) {
							...
							<div class="row fed_dashboard_item_field">
								<div class="col-md-12">
									<div class="fed_header_font_color"><?php esc_attr_e( 'Post Format', 'frontend-dashboard-custom-post' ); ?></div>
									<?php
									// phpcs:ignore
			...
			$html = apply_filters('fed_cp_show_category_tag_post_format', $html ,$post, $post_settings);
			echo $html;
		}
		...
		private function fed_cp_frontend_dashboard_edit_post_by_id($post_id, $menu){
			...
			if (
				...
				<form method="post"
					 ...
					 * _thumbnail_id
					 */
					if( !isset($post_settings['dashboard']['featured_image']) && post_type_supports($post->post_type, 'thumbnail')) {
